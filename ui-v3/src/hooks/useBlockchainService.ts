...
import {
  ...
  Transaction,
  Recipient,
} from "@/services/transactionDataService";
...
export const useBlockchainService = () => {
  ...
  const blockchainService = isDemoMode
    ? new MockBlockchainService()
    : new BlockchainService();
  const transactionDataService = isDemoMode
    ? new MockTransactionDataService()
    : new TransactionDataService();
  const accountBalanceService = isDemoMode
    ? new MockAccountBalanceService()
    : new AccountBalanceService();
  const smartWalletService = isDemoMode
  ...
  const loadRecentData = useCallback(
    async (walletAddress: string, offset: number = 0, limit: number = 20) => {
      setIsLoading(true);
      try {
  ...
  const loadAccountBalances = useCallback(
    async (walletAddress: string) => {
      setIsLoading(true);
      try {
        const balances = await accountBalanceService.getAccountBalances(
          walletAddress
        );
  ...
  const loadSmartWallets = useCallback(
    async (walletAddress: string) => {
      setIsLoading(true);
      try {
  ...
  const loadWalletActivity = useCallback(
    async (walletAddress: string) => {
      setIsLoading(true);
      try {
        const activity = await smartWalletService.getWalletActivity(
          walletAddress
        );
